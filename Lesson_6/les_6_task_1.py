...
task_1 занимает меньше всего места так не используются дополнительные переменные из-за "трюка" array[imax], array[imin]
 = array[imin], array[imax] заметил интересный момент, что переменная "_" не вычисляется :) надеюсь это не мой косяк а
 особенность про которую вы говорили. Если я ошибся , в любом случае узнаю что-то новое :)
"""
